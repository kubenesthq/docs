...
**Not supported by the current release.** The app-layer commands named on this page —
`kubenest deploys`, `kubenest logs` and `kubenest status` — were removed from the command tree and
are frozen scope under decision D1. The released CLI installs and operates the platform only, so
this callout comes off only when a release that lifts that decision is promoted.
