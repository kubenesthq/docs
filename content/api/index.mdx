...
```bash filename="check rate limits"
curl -H "Authorization: Bearer $TOKEN" \
  "https://api.your-domain.com/api/v1/rate-limits" | jq .
```
...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/clusters` | List all clusters in the organization. |
| `POST` | `/clusters` | Register a new cluster or initiate cloud provisioning. |
...
| `GET` | `/clusters/{id}/helm-install-command` | Retrieve the pre-filled Helm install command for the operator. |
| `POST` | `/clusters/{id}/rotate-token` | Issue a new cluster JWT and invalidate the current one. |
| `POST` | `/clusters/{id}/scale` | Change node count (cloud-provisioned clusters only). |
| `GET` | `/clusters/{id}/ssh-key` | Retrieve the SSH private key (cloud-provisioned clusters, admin only). |
...
| `PATCH` | `/projects/{id}` | Update project metadata or resource quotas. |
...
| `GET` | `/apps/{namespace}/{name}/dependents` | List workloads that have `exportRef` entries pointing to components in this app. |
...
| `GET` | `/addon-instances/{id}/dependents` | List app components that have `exportRef` entries pointing to this instance. |
