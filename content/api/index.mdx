...
| `POST` | `/clusters/{id}/register-argocd` | Register a legacy backend-owned workload cluster as a host-ArgoCD target. Returns `409 operator_owned_workload_applications` when the connected operator advertises local workload-Application ownership, because that workload path does not use a host target. |
