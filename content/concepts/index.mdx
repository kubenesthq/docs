...
**Organization** is the top-level tenant for memberships, clusters, cloud credentials, and organization settings. Most teams have one organization; the object exists primarily for multi-tenancy in hosted deployments.
...
**Projects** map one-to-one to Kubernetes namespaces. Their Apps, AddonInstances, registry credentials, and workload-component secrets use that namespace. A project is a namespace boundary — today it is not an enforced permission or capacity boundary, which [Projects](/concepts/projects) states plainly.
...
**Addons** are backing services: PostgreSQL, Redis, Kafka, and anything else packaged as a Helm chart. An Addon can live as a component inside an App or as a standalone instance with its own lifecycle. Current standalone-to-App export wiring limitations are documented on [Addons](/concepts/addons).
...
| **Standalone addon** | An addon instance outside any App, through `/api/v1/addon-instances` |
...
**Stack Templates** are reusable multi-component blueprints with parameter slots. Templates carry `project`, `cluster`, or `global` scope metadata, but the backend does not enforce that metadata as an authorization boundary. See [Stack Templates](/concepts/stack-templates).
...
<Cards>
  ...
  <Cards.Card title="Projects" href="/concepts/projects" description="One namespace per project, lifecycle, registry and component secrets — and what is not enforced today." />
