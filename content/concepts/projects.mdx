---
title: Projects
description: Projects group Apps and Addons in one Kubernetes namespace, with registry and per-component secret views.
---
...
A Project is the unit of grouping in kubenest. Every App and standalone AddonInstance belongs to a project; registry credentials and workload-component secrets can also be attached at this scope. From Kubernetes' perspective, a project is a namespace — its runtime resources share that namespace.
...
When the hub send succeeds, the operator creates the namespace and reports `Ready`. If the send fails, the backend still returns the new `Pending` project record and does not replay the `project_create` event after reconnect. There is no project retry endpoint; an org admin must delete the orphaned record and create it again while the cluster is reachable.
...
The API stores role bindings with three roles — `admin`, `member`, and `viewer` — at three scopes: organization, cluster, and project. The **organization** role is enforced. The cluster- and project-scoped bindings are membership records only: no endpoint consults them when authorizing a request.

The three protected project and cluster destructive operations require the org `admin` role. Most other project and cluster routes check organization membership and pass any role, so a `viewer` credential is not read-only in the way the word suggests — it can create projects, deploy apps, and write secrets. It cannot delete a project, delete a cluster, or change a cluster's node count.
...
| `DELETE /projects/{id}` | The org `admin` role |
| `DELETE /clusters/{id}` | The org `admin` role |
| `POST /clusters/{id}/scale` | The org `admin` role |
| Most other project and cluster operations | Organization membership, any role |

Two exemptions apply to the admin-gated routes. A superadmin passes regardless of org role. A cluster with no organization — a legacy record from before organizations existed — keeps the older any-authenticated-user access, because there is no org role to check.

Scope this to what it is: three roles at one scope, not a permission model. Do not design an access policy that needs a `member` and a `viewer` to differ, because today they do not.
...
## Secrets visible to a project

kubenest has no general project-scoped key/value secret store. There is no `POST /projects/{id}/secrets` endpoint. Two narrower secret surfaces exist:

- Registry credentials can be attached with `POST /projects/{id}/registry-secrets` and inspected through the effective-registry-secret view.
- Application values such as API keys are stored against a workload component with `PATCH /apps/{namespace}/{name}/components/{component}/secrets?project_id=<UUID>`.

`GET /projects/{id}/secrets-overview` combines the registry credentials visible to the project with the names of keys held by each workload component. Secret values are not returned.
