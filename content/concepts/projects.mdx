...
Destructive operations require the org `admin` role. Everything else checks organization membership and passes any role, so a `viewer` credential is not read-only in the way the word suggests — it can create projects, deploy apps, and write secrets. It cannot delete a project, delete a cluster, or change a cluster's node count.
...
| Everything else | Organization membership, any role |
