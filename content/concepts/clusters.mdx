...
sequenceDiagram
    ...
    BE-->>You: cluster_id + connection_token
    Note over Op: operator installed via Helm,<br/>carrying that token
    Op->>Hub: WebSocket CONNECT + JWT (outbound)
    Hub->>BE: cluster connected
...
stateDiagram-v2
    ...
    pending --> connected: operator connects (BYO cluster)
    pending --> provisioning: cloud-provisioned
    provisioning --> awaiting_operator: infrastructure ready
    awaiting_operator --> connected: operator connects
    connected --> disconnected: hub lost the session
    disconnected --> connected: automatic reconnect, exponential backoff
    provisioning --> error: provisioning failed
    connected --> error: operator reported a fatal error
```
...
`DELETE /api/v1/clusters/{id}` is not guarded. It does not check for active projects: deletion proceeds with projects still on the cluster, and their project records are deleted along with the cluster record. Check what is running on a cluster before you delete it, because the API will not check for you.
...
- **BYOC registrations** have only their record deleted. Nothing on the cluster is touched — the operator keeps running and must be removed by hand:
