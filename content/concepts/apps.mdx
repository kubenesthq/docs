...
flowchart TD
    ...
    B --> C["Hub routes a CREATE event to the operator"]
    C --> D["Operator writes the StackDeploy CR"]
    D --> E["Operator reconciles:<br/>Helm values → Git → ArgoCD → Kubernetes"]
    E -- "StackDeploy.status.phase" --> F["Hub → Backend → SSE"]
```
