...
- **Standalone addon** — an addon instance that belongs to no App, created through `/api/v1/addon-instances` and shared by several Apps. Wire it with `export_ref.addon_instance_id` instead of `export_ref.component`.
...
2. The addon's chart installs, and the operator extracts its exports. For type `postgres` these
   are `dsn`, `host`, `port`, `user` and `database`, plus `passwordRef.name` and
...
3. The StackDeploy controller resolves each `valueFrom.exportRef` against those exports, in
   dependency order, and writes the resolved value onto the Workload's env. A reference that
   cannot resolve yet — the addon is still deploying — is kept pending and retried on the next
   reconcile.

The `dsn` is a full PostgreSQL URL whose password segment is a `${...}` placeholder resolved from
the Secret named by `passwordRef`, so the password itself never sits in the addon's exports.
