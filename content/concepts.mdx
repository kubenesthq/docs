...
A cluster arrives by installing the KubeNest platform bundle onto machines you supply
([Install](/install)). Every cluster registers to a control plane — the first install creates one
inside the cluster it is installing — and connecting a cluster you already run is not supported yet
([Connect a cluster](/connect-cluster)).
...
data. Every component of an App runs on that App's cluster; nothing is placed across clusters.
