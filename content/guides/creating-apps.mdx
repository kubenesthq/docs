...
The `export_ref.export_key` value `connection_string` is a kubenest convention for addons that have an `AddonDefinition` entry. The available keys for each addon type are listed under [Addons](/concepts/addons).
