...
`db_password` is omitted — the `generator` handles it. The backend resolves all parameters, writes the final values, and dispatches the deploy event to the operator.

The body takes `project_id`, `parameters` and `timeout` and nothing else — it rejects unknown fields, so there is no way to choose the App's name here. The deploy is named for you, as the template name followed by the first eight characters of the project UUID (`postgresql-16-b2c3d4e5` for the call above). The response carries that `deploy_name` and the namespace it landed in.
...
APP_RESPONSE=$(curl -s -H "Authorization: Bearer $TOKEN" \
  "https://api.your-domain.com/api/v1/apps/my-project/postgresql-16-b2c3d4e5?project_id=$PROJECT_ID")
...
2. A **Helm values file** committed to the GitOps repository at `clusters/{id}/namespaces/{ns}/apps/postgresql-16-b2c3d4e5/postgres/values.yaml`.
...
curl -X PATCH "https://api.your-domain.com/api/v1/apps/my-project/postgresql-16-b2c3d4e5?project_id=$PROJECT_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "components": [
...
A `components` entry replaces that component wholesale rather than merging field by field, so send the addon's complete spec with only the version changed — see [Creating apps](/guides/creating-apps#updating-an-app).
