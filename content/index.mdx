...
**Not supported by the current release.** `kubenest deploy` and the rest of the app layer were
...
**You never upload a kubeconfig.** The agent runs inside your cluster with its own in-cluster
access, and application, addon and component-secret operations flow through the hub to it as
signed, schema-validated events.

Every workload is deployed by the agent on its own cluster: the control plane sends the intent
through the hub, and the agent on that cluster creates the Argo CD Application. A stack's components
all run on the stack's cluster. [Architecture](/architecture) has the detail.
...
- [Connect a cluster](/connect-cluster) — existing clusters are not supported yet, and why
