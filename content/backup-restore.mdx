...
[HA tiers page](/ha) treats snapshots as the thing that makes the single-server tier
...
Datastore snapshot freshness is watched on the same terms, so the datastore group of the health report carries a collection error and the
...
**Read access to the backup bucket exposes cluster secrets.** Sealed material and the key that
opens it are both in there, so whoever can list and read the bucket can read them. Treat the bucket
itself as secret material, not as a place to hand a whole team `s3:GetObject`.

**Scope the credentials per cluster.** One credential per cluster, reaching only that cluster's
prefix, so a leaked key exposes one cluster instead of the fleet. `kubenest backup set-target`
takes `--prefix` as the cluster root and writes workload backups under `<prefix>/workload/` and
datastore snapshots under `<prefix>/datastore/`, which is the boundary to write the access policy
against.
</Callout>
...
The proof set proves the mechanism. **It does not prove your data.** A drill that only ever
restores the same synthetic Pod is answering an easier question than the one you are paying it to
answer: it proves the backup was written, that object storage still holds it, and that the restore
path puts it back — compared byte-for-byte, through the same node-agent/Kopia path a customer PVC
takes. It does not prove that a particular namespace of yours can be restored, and this page does
not claim it does.

The result goes into fleet telemetry and is visible against the cluster. **A failed drill is an
alert, not a log line.** It is also a gate: the [upgrade](/upgrades) pre-flight refuses to
run when the last drill did not pass, because rollback depends on restore working.

<Callout type="info">
**This path is exercised against real storage.** The Platform 1.0 release gate created a Velero
file-system backup, restored the proof objects and PVC bytes in 33 seconds, then replaced the
...
<Callout type="warning">
**Not supported by the current release.** `kubenest backup restore` is registered but not built —
running it exits with a "not yet implemented" error. The command arrives with **Bundle 1.2, on
promotion**; until then this is how it will behave, not something you can run today.
...
<Callout type="warning">
**Not supported by the current release.** This step is `kubenest backup restore` on top of the
rebuilt cluster, and that command is not built yet — it arrives with **Bundle 1.2, on promotion**.
The datastore half above it, `kubenest platform restore`, is implemented and is the tested disaster
path.
...
Run the [install verification checks](/install#verifying-the-install), confirm all etcd
