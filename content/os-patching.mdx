...
**KubeNest does not sell, require or configure kernel livepatching.** Enrolling your hosts would
mean a commercial subscription and a credential in the install flow, and that is your relationship
to have, not ours to broker.
...
**Not supported by the current release.** The CLI half of this table is `kubenest node reboot`,
which is not in the command tree yet; it arrives with **Bundle 1.2, on promotion**. Fleet telemetry
observes the reboot today.
...
One cluster, one window, one name: `kubenest cluster set-window` writes the setting that both the
bundle upgrade and this reboot orchestration read. Two names would mean two windows — a cluster
that reboots at 02:00 and upgrades at 03:00 by different rules — which is exactly what the single
...
arrives with **Bundle 1.2, on promotion**, and until then no released CLI has this verb.
