...
## Handing workload Application ownership to the cluster

Earlier registered installs can have workload Applications owned by the control plane. A normal
bundle upgrade preserves every existing Helm value, including an explicit
`kubenest.workloadApplications.enabled: false`; it does not silently change who deploys your
workloads.

When you are ready to move that ownership to the in-cluster operator, request the explicit
hand-off through the upgrade command:

```bash filename="terminal"
kubenest platform upgrade --cluster prod-1 --to <current-or-newer-bundle> \
  --migrate-workload-applications
```

The CLI first asks the control plane to detach its host Applications and refuses unless every
result is detached or already absent. It then changes only the ownership gate in the existing
agent HelmChart; the cluster identity, agent token, Git credential, and every other Helm value are
preserved. Finally it reads the operator's own gate record and requires it to report `open`.

The explicit migration is the one exception to the same-bundle refusal: it may use the current
bundle because ownership, rather than a component version, is the requested change. Without this
flag a same-bundle upgrade remains refused. If the hand-off cannot complete, the control plane
remains in the intermediate migration state and the CLI names the failed step; it never opens the
operator gate before host Applications are detached.
