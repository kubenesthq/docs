...
in the [bundle manifest](/bundle), and you can see the diff before you start:
...
overhead plus a per-node cost, both from the manifest — and refuses to start an upgrade that cannot
finish inside it. Starting one you know will not fit is choosing the overrun in advance.
...
| 6 | `host-policy` | Ubuntu's APT policy — security updates only, `Automatic-Reboot "false"` — and the `kubenest.io/auto-reboot=false` hold, on every node. A fresh install writes both; a cluster that predates 1.2 gets them here |
| 7 | **`kubernetes`** | **Point of no return.** k3s on each node in turn, via `system-upgrade-controller` |
| 8 | `verify` | Full post-upgrade verification |
| 9 | `record` | The cluster's bundle version is updated |
...
flowchart TD
    ...
    S5 --> S5b["6 · host policy"] --> S6["7 · kubernetes<br/>node by node"]
    S6 --> S7["8 · verify"] --> S8["9 · record"]
...
The diagram is the argument for the ordering. Every stage before 7 exits through the cheap path on
the left; only 7 and 8 exit through a datastore restore. Five of the seven ways an upgrade can
...
Stage 6 is the host step, and it is the only stage that exists for clusters that predate the bundle
being installed on them. This page and the installer write the same two things to a fresh install's
hosts, so a fresh cluster has nothing to do here: Ubuntu's APT policy — security updates only,
`Automatic-Reboot "false"`, and k3s held out of `needrestart`'s reach — and the
`kubenest.io/auto-reboot=false` hold, which is what makes `kubenest node reboot` rather than luck
the thing that decides when a node reboots. A cluster upgraded from an earlier release gets both
here, before Kubernetes moves.

Two properties matter operationally. It is **idempotent**: a re-run on a cluster that already
carries the policy writes nothing and reports no change. And it is **reversible per node**: servers
keep the hold permanently, because in 1.2 a server never reboots by itself, while an agent's hold
is restored to the value it had before the upgrade — so an agent that rebooted automatically in its
window still does, and one that was already held when the upgrade started keeps its hold.

Stage 7 upgrades nodes one at a time. Each node is cordoned, drained, upgraded, and must return to
...
| **`single-server`, enough Ready replicas already on agent nodes** | No downtime for the workload itself while those replicas remain healthy. A two-node 0.9 → 1.0 upgrade — the last transition that moved Kubernetes — kept a two-replica workload reachable on all 146 one-second probes; that result does not remove the placement condition | Restarts once, when its node drains |
...
upgraded on three machines — see [high availability](/ha).
...
[profiles](/bundle#profiles). And the agent moved at stage 5 is not part of the component
...
[backup and restore](/backup-restore) — but note that on `single-server` the restore is
...
### Host policy — left in place

One thing a rollback does not undo is the host step (stage 6). The APT policy under
`/etc/apt/apt.conf.d` and the servers' `kubenest.io/auto-reboot=false` hold stay exactly as the
upgrade left them, whichever mechanism the rollback uses.

That is deliberate. Reverting the drop-in would restore Ubuntu's own update behaviour — security
updates installed at a random time each day, and a reboot the node chooses for itself — which is
precisely what the host step exists to replace. Reverting the hold would let a server reboot by
itself, which 1.2 does not promise.

Nothing about a rollback depends on that stage having run, either: a cluster that failed before
stage 6 rolls back with nothing extra to undo.
...
The one exception we intend to offer is narrower than "automatic upgrades": security releases
applying themselves inside your maintenance window, with everything else still waiting for a
person. A release that exists only to carry a patched component is the case where waiting is the
bigger risk, and it is also the case where the change is smallest.
...
pinned by the bundle you name, and only that one — so a bundle you jump over is never scanned
against, and an API removal it carries goes unseen. Skipping a bundle is skipping the check that
makes the upgrade safe, which is why the answer is a gate rather than advice.
