...
embedded etcd, changed the live value, stopped k3s, restored the named S3 snapshot, and recovered
the pre-change value in 18 seconds. That is the tested disaster path; checking that a snapshot
object merely exists is not accepted as restore evidence.
