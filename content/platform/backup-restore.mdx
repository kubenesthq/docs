...
**The target is proven, not assumed.** `set-target` waits until Velero has reached the bucket with
those credentials and marked the storage location `Available`, and only then writes the default
backup schedule. A target that does not work fails here, rather than the first time you need it.
...
<Callout type="warning">
**Nothing tells you about it yet.** The design is that the cluster reports `backup: unconfigured`
in every telemetry heartbeat and the console shows a warning until it is resolved. The heartbeat
carries no backup state today — it reports node and resource data only — so an unconfigured backup
is currently silent, which is exactly the failure mode this page exists to prevent. Tracked as
`kn-j5s`.
</Callout>

The principle behind that design: a cluster that has never taken a backup is exactly the quiet
failure this product exists to prevent, so it should be surfaced continuously rather than block the
install. Until the heartbeat carries it, check the target yourself after installing.
