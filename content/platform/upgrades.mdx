...
| **Node readiness** | Every node reports Ready | Upgrading onto an already-degraded cluster turns one problem into two |
| **Disk headroom** | Every node has room for new images alongside the old ones | Running out of disk mid-upgrade is a hard failure at the worst moment |
| **Pod disruption budgets** | No PDB would block a drain indefinitely | A PDB that permits zero disruption stalls the upgrade forever, holding the cluster mid-transition |
...
and etcd has written data in the new schema, going back means restoring the etcd snapshot taken at
stage 2.
...
An etcd restore rolls back *cluster state* — the Kubernetes objects. Your persistent volumes are
...
with no data implications, and never automatic for the Kubernetes stage, where it means an etcd
restore and a service interruption that nobody should trigger without a human deciding.
</Callout>
...
- **Resume** once the failing node's problem is fixed. This is the normal path and usually the
  right one.
- **Restore** to N from the stage-2 snapshot, accepting the service interruption.
...
No pod in CrashLoopBackOff or Pending. Component versions match the target manifest.
