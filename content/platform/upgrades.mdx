...
**Step through one bundle at a time.**

Nothing currently refuses a target more than one bundle ahead. The bundle-path gate checks that
the target is newer than what you run and that it offers your tier and your profiles — not that
the hop from your version to it is one we have tested. Only two bundles exist so far, so there is
nothing to skip yet, but the gate will not stop you when there is.

The reason to step is the deprecated-API scan. It runs against the Kubernetes version pinned by
the bundle you name, and only that one. An API removed in 1.1 and irrelevant again by 1.3 still
breaks your workloads at 1.1, and a 1.0 → 1.3 jump would never scan for it.
