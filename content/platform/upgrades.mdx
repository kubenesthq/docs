...
<Callout type="info">
The core install places the pinned `system-upgrade-controller` and waits for it to become Ready,
but it creates no upgrade Plan and runs none of the workflow below. The installer's proven journal
and resume path cover installation only; an upgrade needs its own transition journal.
</Callout>
...
| **`single-server`, replicas only on agent nodes** | No downtime for the workload itself | Restarts once, when its node drains |
