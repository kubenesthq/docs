...
`kubenest backup set-target` — see [Backup and restore](/platform/backup-restore).
...
One inbound path is still required today, and it is easy to miss when planning firewall rules: the
control plane connects to your cluster's Kubernetes API server directly for some operations, using
the credentials the agent advertises. That API endpoint must be reachable from the control plane.
Removing that requirement is tracked as `kn-cjqw`; see
[Architecture](/architecture#kubenest-backend).
