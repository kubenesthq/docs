...
<Callout type="warning">
**The reporting half of this is not built yet.** The cluster does not report
`backup: unconfigured` in its heartbeat, and the console raises no warning for it: the heartbeat
carries node, version and resource data, and the cluster record has no field for backup state at
all. Until that lands, an unconfigured backup is visible once — in the install output — and not
continuously.
</Callout>

A cluster that has never taken a backup is exactly the quiet failure this product exists to
prevent. The intent is to surface that continuously rather than block the install on it; today
only the second half of that holds.
...
**`--ha ha` has never been run on real hardware.** Stage 3 joins the second and third servers,
preflight checks the etcd peer ports and the node count, and both are unit-tested — but every
install performed against real machines so far has been `single-server`. Treat the three-node
tier as written and unproven.
</Callout>
...
<Callout type="warning">
**No component profile installs yet.** `observability`, `secrets` and `replicated-storage` are
pinned in the manifest and marked provisional there, but none of them is built. Stage 11 refuses
a profile it cannot install rather than installing core and reporting success — a cluster that
does not match its own record is the thing the record exists to prevent. Install core now and add
the profile when it ships.

`ha` is the exception: it is a topology rather than a component set, and stage 3 performs it.
</Callout>
...
<Callout type="warning">
**The console side of `login` does not exist yet.** By default the CLI starts a device
authorization and prints a code to approve in the console — but the page that approves it has not
been built, and neither has the CLI-token screen the `--token-stdin` fallback tells you to create
a token on. The control plane issues and accepts the tokens; there is currently no screen in the
console that produces one.
</Callout>
...
bundle version plus a recorded profile set. **Platform 1.0 is the only released bundle**; a
...
One of those two shapes has been measured. A single-server core install on a 4 vCPU / 8 GB Ubuntu
24.04 host completed all thirteen stages in **4m14s**, and the release test asserts the budget on
that shape. The three-node-with-every-profile shape has never been run: no install has been
performed on three machines, and no profile is installable. Its budget is a target that nothing
yet measures.
...
the heartbeat, and backup configuration state is not reported anywhere yet.
