...
**KubeNest does not write that policy yet — it inherits Ubuntu's.** The install writes no APT
configuration today, so what you get is whatever the machine image already does. On a stock Ubuntu
24.04 cloud image that is unattended security upgrades enabled by default, which is close to the
policy above and arrives at it by accident. The parts that are not accidental are the parts that
are missing: nothing holds k3s, and nothing stops the image's own reboot behaviour. The policy
above is decided and lands with the reboot coordinator.
</Callout>
