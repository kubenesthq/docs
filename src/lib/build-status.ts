 ...
 * be removed.
 *
 * Verified against the repos on 2026-08-23 (PLAN-BUILD-2026-08-20.md §1 and §2a),
 * not inferred from bead titles. Do not add an entry without checking the code.
 */
...
export const SPEC_PAGES: Record<string, BuildStatusEntry> = {
  ...
  '/platform/bundle': {
    ...
    availability: 'partial',
    missing:
      '"Tested together as one thing" is still the aspirational half. The compatibility matrix has never run, so none of the five configurations has been through install, workload, backup, restore drill and upgrade on a release; both component profiles in the manifest are marked provisional and neither has been tested against core. The limits are still marked provisional, and the two figures real hardware has now measured have not replaced any of them. The CVE response and the agent compatibility window this page now states are decided policy, not shipped behaviour: no advisory feed meets the two-day clock (kn-eewa) and nothing enforces the agent window (kn-opj8).',
    today:
  ...
  '/platform/profiles': {
    beads: ['kn-ynaq', 'kn-sev5'],
    availability: 'unavailable',
    missing:
      'No component profile installs anything, and asking for one fails the install: stage 11 refuses `observability` and `secrets` rather than installing core in their place, so the run stops with core already on the machines and the cluster marked install-failed. Nothing adds a profile to a cluster afterwards either — decision P settles that it will be an upgrade when it is built (kn-avk0), which is design rather than code. Neither profile has declared a backup set yet (kn-436i).',
    today:
      'Core installs, and the profile set is recorded and validated as of kn-boj — an unknown profile is rejected with 422 rather than ignored. `ha` is the exception in the list: it is an install-time topology stage 3 builds, not components stage 11 installs. The list itself is settled as of decisions M and N: `secrets` resolves to sealed-secrets alone, and `replicated-storage` is out of 1.x rather than pinned untested.',
  },
  '/platform/upgrades': {
    beads: ['kn-1krv', 'kn-kp3', 'kn-nqj', 'kn-mtpf', 'kn-tczn'],
    availability: 'partial',
    missing:
      'Two things on this page have never run on hardware, and they are the two that matter when an upgrade goes wrong. The datastore-restore rollback — recovery from a failure *after* the point of no return, which is what every pre-flight gate and the stage-2 backup are insurance against — has only ever run in unit tests (kn-1krv). And no upgrade has run on the three-node `ha` tier, so the drain-and-return behaviour described for a multi-node control plane is untested (kn-kp3). Separately, the maintenance window is not in force at all: `kubenest cluster set-window` stores it through a control-plane route that does not exist, and the upgrade never reads one back, so the gate passes every cluster as unconfigured and the mid-upgrade pause never fires (kn-nqj), and the window-fit estimate this page describes needs that window before it can be computed (kn-ght1). Two rules the page now states as decided are not enforced: nothing refuses a hop of more than one bundle (kn-mtpf) and the deprecation scan reads live objects only, never the Git desired state (kn-tczn).',
    today:
  ...
  '/platform/os-patching': {
    ...
    availability: 'unavailable',
    missing:
      'The patching and reboot policy is decided (decisions W, X, Y) and none of it is built. `unattended-upgrades` is never configured by the installer, kured watches a platform sentinel file that nothing yet creates, so no reboot is ever coordinated, and reboot windows do not exist — neither `kubenest cluster set-reboot-window` nor `kubenest node reboot` is a command. The cluster record carries no patch or reboot state, so the seven-day warning and fourteen-day critical thresholds the page states have nothing to evaluate.',
    today:
